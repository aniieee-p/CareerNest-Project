            ...
            <div className="max-w-4xl mx-auto my-10 px-4" style={{ color: "var(--cn-text-1)" }}>
                ...
                {loading ? (
                    ...
                    <div className="text-center py-32">
                        <p className="text-lg font-semibold mb-2" style={{ color: "var(--cn-text-1)" }}>Job not found</p>
                        <p className="text-sm mb-4" style={{ color: "var(--cn-text-3)" }}>This listing may have been removed or is no longer available.</p>
                        <button onClick={() => navigate("/jobs")} className="px-4 py-2 rounded-xl text-sm font-medium text-white" style={{ background: "linear-gradient(135deg,#27bbd2,#6366f1)" }}>
                            Browse Jobs
                        </button>
                    </div>
                ) : (
                        ...
                        <div className="rounded-2xl border p-6 mb-6" style={{ background: "var(--cn-jd-card)", borderColor: "var(--cn-border-subtle)", boxShadow: "var(--cn-card-shadow)" }}>
                            <div className="flex items-start justify-between gap-4 flex-wrap">
                                ...
                                {user?.role !== "recruiter" && (
                                    ...
                                    >
                                        {applyLoading
                                            ? <span className="flex items-center gap-2"><Loader2 size={14} className="animate-spin" /> Applying...</span>
                            ...
                            <div className="flex items-center gap-2 flex-wrap mt-5">
                                ...
                                <Badge className="border-0 text-[#6366f1] flex items-center gap-1" style={{ background: "rgba(99,102,241,0.1)" }}>
                                    <IndianRupee size={11} />{singleJob?.salary != null ? `${singleJob.salary} LPA` : "N/A"}
                                </Badge>
                            </div>
                        </div>
                        ...
                        <div className="rounded-2xl border p-6" style={{ background: "var(--cn-jd-card)", borderColor: "var(--cn-border-subtle)", boxShadow: "var(--cn-card-shadow)" }}>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                                {[
                                    ...
                                    { icon: Clock,       label: "Experience", value: singleJob?.experienceLevel != null ? `${singleJob.experienceLevel} yrs` : "N/A" },
                                    { icon: IndianRupee, label: "Salary",     value: singleJob?.salary != null ? `${singleJob.salary} LPA` : "N/A" },
